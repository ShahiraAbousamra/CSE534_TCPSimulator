...
public class analysis_pcap_http {
	...
	public static String packetFilename = "";
	public static String flowFilename = "";
	public static String transactionFilename = "";
	public static String retransmissonsFilename = "";
	public static String httpFilename = "";
	public static String httpStatsFilename = "";
	public static FileWriter packetFileWriter;
	public static FileWriter flowFileWriter;
	public static FileWriter transactionFileWriter;
	public static FileWriter retransmissonsFileWriter;
	public static FileWriter httpFileWriter;
	public static FileWriter httpStatsFileWriter;
	public static PrintWriter packetPrintWriter ;
	public static PrintWriter flowPrintWriter;
	public static PrintWriter transactionPrintWriter;
	public static PrintWriter retransmissonsPrintWriter;
	public static PrintWriter httpPrintWriter;
	public static PrintWriter httpStatsPrintWriter;
	...
	public static MyTCPPacket parsePacket(byte[] packetHeader, byte[] packetPayload, byte[] sourceIP, byte[] destinationIP, long totalPacketSize, long captureTS_ms){
		...
		myPacket.tcpPayload = packetPayload;
	...
	public static boolean updateFlows(MyTCPPacket myPacket, ArrayList<MyTCPFlow> myFlows){
		...
		if(myPacket.SYN && !myPacket.ACK){ 		// A new connection establishment 
			...
			flow.startTS_srcToDest = myPacket.captureTS_ms;
			flow.start_captureTS_ms = myPacket.captureTS_ms;
			flow.end_captureTS_ms = myPacket.captureTS_ms;
...
//					currentCWnd.size = prevCWnd.size;
	...
	public static void getHttpRequestResponse(MyTCPFlow flow, PrintWriter httpFile){
		
		writeToFile(httpFile, "\n\nRequest Number = " + flow.flowNum 
				+ "\n-----------------------------"
				+ "\n");
		...
		for(int i=0; i<flow.packets.size(); i++){
			...
			if(packet.payloadLength <= 0) // ignore Syn and Ack packets
				continue;
			if(packet.fromClient) // Request
			{
				String httpRequest = new String(packet.tcpPayload);
				writeToFile(httpFile, "\n\tRequest = \n" + httpRequest);
				writeToFile(httpFile, "\tResponse = \n" );
				
					
			}
			else if(!packet.fromClient ){ // Response
				String httpResponse = new String(packet.tcpPayload);
				writeToFile(httpFile, httpResponse);
	...
	public static void getHttpVersion(ArrayList<MyTCPPacket> packets, ArrayList<MyTCPFlow> flows, PrintWriter httpFile){
		long maxSequenceNumber = 0;
		long retransmissionCount = 0;
		...
		int numberOfParallelConnections = 1;
		MyTCPFlow flow0 = flows.get(0);
		// Get the re-transmitted packets
		for(int i=1; i<flows.size(); i++){
			MyTCPFlow flowX = flows.get(i);
			if(flowX.start_captureTS_ms <= flow0.start_captureTS_ms 
					&& flowX.end_captureTS_ms > flow0.start_captureTS_ms
					||
					flowX.start_captureTS_ms > flow0.start_captureTS_ms 
					&& flowX.start_captureTS_ms <= flow0.end_captureTS_ms 
					)
				numberOfParallelConnections++;
		}
		writeToFile(httpFile, "Number of Parallel Connections = " + numberOfParallelConnections
				+"\n");
		//System.out.println("number of parallel conn = " + numberOfParallelConnections);

		// Check if it contains parallel requests/response in flight
		boolean MultipleRequestsInFlight = false;
		boolean isEncrypted = true;
		boolean openRequest = false;
		for(int i=0; i<flows.size(); i++){
			MyTCPFlow flow = flows.get(i);
			for(int j=0; j<flow.packets.size(); j++){
				MyTCPPacket packet = flow.packets.get(j);				
				if(packet.payloadLength == 0)
					continue;
				if(packet.fromClient){
					String s = new String(packet.tcpPayload);
					if(s.toLowerCase().substring(0, 2).equals("get"))
						isEncrypted = false;
					if(openRequest){
						MultipleRequestsInFlight = true;
						break;
					}
					else
						openRequest = true;
					...
					String s = new String(packet.tcpPayload);
					if(s.toLowerCase().substring(0, 2).equals("http"))
						isEncrypted = false;
					if(!openRequest)
						continue;
					if(packet.PSH)
			...
			if(MultipleRequestsInFlight)
				break;
			
		}

		if(MultipleRequestsInFlight)
			writeToFile(httpFile, "Protocol supports multiple requests/responses in flight" 
					+"\n");
		else
			writeToFile(httpFile, "Protocol does not support multiple requests/responses in flight" 
					+"\n");
		
		if(numberOfParallelConnections == 1 /*&& MultipleRequestsInFlight && isEncrypted*/)
			writeToFile(httpFile, "Http Version = 2.0" 
					+"\n");
		else if(numberOfParallelConnections > 1){
			if(MultipleRequestsInFlight){
				writeToFile(httpFile, "Http Version = 1.1" 
					+"\n");
			}
			else //if(!isEncrypted)
				writeToFile(httpFile, "Http Version = 1.0" 
					+"\n");
		}
		
		// Page Load Time
		long pageLoadTimeInMS = packets.get(packets.size()-1).captureTS_ms - packets.get(0).captureTS_ms;
		writeToFile(httpFile, "\nPage Load Time (ms) = " + pageLoadTimeInMS   
				+"\n");
		
		// Number of Packets
		long packetsFromServerCount = 0;
		long packetsFromClientCount = 0;
		long packetsFromServerSize = 0;
		long packetsFromClientSize = 0;
		long packetsFromServerPayloadSize = 0;
		long packetsFromClientPayloadSize = 0;
		for(int i=0; i<packets.size(); i++){
			MyTCPPacket packet = packets.get(i);
			if(packet.fromClient){
				packetsFromClientCount++;
				packetsFromClientSize += packet.totalPacketSize;
				packetsFromClientPayloadSize += packet.payloadLength;
			}
			else{
				packetsFromServerCount++;
				packetsFromServerSize += packet.totalPacketSize;
				packetsFromServerPayloadSize += packet.payloadLength;
				
			}
			
		}
		writeToFile(httpFile, "\nNumber of Packets from Server = " + packetsFromServerCount
				+ "\nNumber of Packets from Client = " + packetsFromClientCount
				+ "\nTotal Number of Packets = " + packets.size()
				+"\n");
			

		writeToFile(httpFile, "\nTotal Size of Packets from Server (bytes) = " + packetsFromServerSize
				+ "\nTotal Size of Packets from Client (bytes) = " + packetsFromClientSize
				+ "\nTotal Size of Packets Exchanged (bytes) = " + (packetsFromServerSize + packetsFromClientSize) 
				+"\n");

	
		writeToFile(httpFile, "\nTotal Size of Payload from Server (bytes) = " + packetsFromServerPayloadSize
				+ "\nTotal Size of Payload from Client (bytes) = " + packetsFromClientPayloadSize
				+ "\nTotal Size of Payload Exchanged (bytes) = " + (packetsFromServerPayloadSize + packetsFromClientPayloadSize) 
				+"\n");
		
	}

	public static void main(String[] args) throws IOException {
		// Get the pcap filepath
		if(args.length <= 0){
			System.out.println("PCap Filepath missing");
			return;
		}
		final String filepath = args[0];
		System.out.println("PCap Filepath = " + filepath);
		...
		httpFilename = filepath.substring(0, filepath.length()-5) + "_http_" + dateFormat.format(date) + ".txt";
		httpStatsFilename = filepath.substring(0, filepath.length()-5) + "_http-stats" + dateFormat.format(date) + ".txt";
...
//			retransmissonsFileWriter = new FileWriter(retransmissonsFilename, true);
			httpFileWriter = new FileWriter(httpFilename, true);
			httpStatsFileWriter = new FileWriter(httpStatsFilename, true);
...
//			retransmissonsPrintWriter = new PrintWriter(retransmissonsFileWriter);
			httpPrintWriter = new PrintWriter(httpFileWriter);
			httpStatsPrintWriter = new PrintWriter(httpStatsFileWriter);
		} 
		catch (IOException e) {
			e.printStackTrace();
			throw e;
		...
		ArrayList<MyTCPFlow> flows = new ArrayList<MyTCPFlow>(); 
...
//	                System.out.printf("frame #%d%n", packet.getFrameNumber());  
		...
		for(int i=0; i<flows.size(); i++){
			MyTCPFlow flow = flows.get(i);
			getHttpRequestResponse(flow, httpPrintWriter);
...
//			writeToFile(flowPrintWriter, "\n"); 
		...
		getHttpVersion(packets, flows, httpStatsPrintWriter);
		
		

//		pcap.dispatch(10, new ByteBufferHandler<StringBuilder>() {
		...
		pcap.loop(10, new JPacketHandler<StringBuilder>() {  
            ...
            public void nextPacket(JPacket packet, StringBuilder errbuf) {  
                ...
                if (packet.hasHeader(Tcp.ID)) {  
                	...
                	packet.getHeader(ip);
                	MyTCPPacket myPacket = parsePacket(TCP_header, TCP_Payload, ip.source(), ip.destination());
                	packets.add(myPacket);
                	updateFlows(myPacket, flows);
            		writeToFile(packetPrintWriter, myPacket.toString());
                }  
